...
int _filemgr_aio_init(struct async_io_handle *aio_handle)
{
	...
	void *buf;
	malloc_align(buf, FDB_SECTOR_SIZE, aio_handle->block_size * aio_handle->queue_depth);

	aio_handle->aio_buf = (uint8_t *) buf;
	aio_handle->offset_array = (uint64_t *) malloc(sizeof(uint64_t) * aio_handle->queue_depth);

#if JB_DEV
    void *iov_buf;
	aio_handle->iovecs = (struct iovec *)calloc(aio_handle->queue_depth, sizeof(struct iovec));
	aio_handle->cqe_results = (struct io_uring_cqe **)calloc(aio_handle->queue_depth, sizeof(struct io_uring_cqe *));
	
	for(i = 0; i < aio_handle->queue_depth; i++){
		if (posix_memalign(&iov_buf, aio_handle->block_size, aio_handle->block_size)){
			printf("AIO INIT buffer posix_memalign failed\n");
			return FDB_RESULT_AIO_INIT_FAIL;
		}
		aio_handle->iovecs[i].iov_base = iov_buf;
		aio_handle->iovecs[i].iov_len = aio_handle->block_size;
	}
	
	int rc = io_uring_queue_init(aio_handle->queue_depth, &aio_handle->ring, 0);
	if (rc < 0){
		return FDB_RESULT_AIO_INIT_FAIL;
	} 
#else	// JB_DEV
	aio_handle->ioq = (struct iocb**) malloc(sizeof(struct iocb*) * aio_handle->queue_depth);
	aio_handle->events = (struct io_event *) calloc(aio_handle->queue_depth, sizeof(struct io_event));
                           ...
                           size_t read_size, uint64_t offset)
{
	...
	sqe = io_uring_get_sqe(&(aio_handle->ring));
	
	if (!sqe){
		printf("io_uring_get_sqe failed, ring size : %lu\n", aio_handle->ring.sq.ring_sz);
		return FDB_RESULT_INVALID_ARGS;
	}

	io_uring_prep_readv(sqe, aio_handle->fd, 
						&(aio_handle->iovecs[aio_idx]),
						1,
						(offset / aio_handle->block_size) * aio_handle->block_size);
	// Record the original offset.
	aio_handle->offset_array[aio_idx] = offset;
	io_uring_sqe_set_data(sqe, &aio_handle->offset_array[aio_idx]);
...
#if JB_DEV
	//baek
	printf("baek -- _filemgr_aio_submit (io_uring)\n");
    if (!aio_handle) {
        return FDB_RESULT_INVALID_ARGS;
    }
	int rc = io_uring_submit(&(aio_handle->ring));

	printf("baek -- rc (IO submitted number) : %d\n", rc);
	if (rc < 0) {
...
#else // JB_DEV
	//baek
	printf("baek -- _filemgr_aio_submit (libaio)\n");
                           ...
                           int max, unsigned int timeout)
{
#ifdef _ASYNC_IO
	//baek
	printf("baek -- _filemgr_aio_getevents\n");
#if JB_DEV
    if (!aio_handle) {
        return FDB_RESULT_INVALID_ARGS;
    }
	int num_events = io_uring_peek_batch_cqe(&(aio_handle->ring), aio_handle->cqe_results, max);
	printf("baek -- io_uring_peek_batch_cqe submitted : %d, result : %d\n", max, num_events);
	io_uring_cq_advance(&(aio_handle->ring), num_events);
	...
	if (err){
		printf("baek -- __io_uring_peek_cqe failed\n");
		return FDB_RESULT_AIO_GETEVENTS_FAIL;
	}

	//int num_events = io_uring_wait_cqe(&(aio_handle->ring), &cqe);
   	//printf("baek -- io_uring_wait_cqe return num_events : %d\n", num_events);

	if (num_events < 0) {
        return FDB_RESULT_AIO_GETEVENTS_FAIL;
    }

	printf("baek -- cqe->res : %d\n", cqe->res);
	io_uring_cqe_seen(&(aio_handle->ring), cqe);
    return num_events;*/
...
#if JB_DEV
	io_uring_queue_exit(&(aio_handle->ring));
